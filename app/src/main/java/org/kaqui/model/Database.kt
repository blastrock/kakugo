...
class Database constructor(context: Context, val database: SQLiteDatabase) {
    ...
    data class DayStatistics(val timestamp: Long, val askedCount: Int, val correctCount: Int, val uniqueAskedCount: Int, val uniqueCorrectCount: Int)

    fun getAskedItem(): List<DayStatistics> {
        commitAllSessions()

        val stats = mutableListOf<DayStatistics>()
        database.query(SESSIONS_TABLE_NAME, arrayOf("start_time", "item_count", "correct_count", "unique_item_count", "unique_correct_count"), null, null, null, null, "start_time").use { cursor ->
            while (cursor.moveToNext())
                stats.add(DayStatistics(cursor.getLong(0), cursor.getInt(1), cursor.getInt(2), cursor.getInt(3), cursor.getInt(4)))
        }
        return stats
    }
